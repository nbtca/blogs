...
categories: 
  - 技术
  - Python
cover: ./Python爬虫实战-获取王者荣耀英雄大全.assets/data-mining.png
...
![python-wzry-1](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-1.png)
...
![python-wzry-2](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-2.png)

拼接li标签下对应的链接(https://pvp.qq.com/web201605/ + herodetail/***.html)，就可以访问到指定英雄对应的链接，如下所示：

![python-wzry-3](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-3.png)

拼接的URL和访问的页面如下所示：

![python-wzry-4](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-4.png)

![python-wzry-5](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-5.png)
...
![python-wzry-6](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-6.png)

下面访问单个英雄的链接，理清HTML结构后就可以获取数据。

英雄的称号在h3标签中：

![python-wzry-7](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-7.png)

英雄的属性在class="cover-list"的标签中，每个属性的信息在li标签中，每个属性的值其实就是横条的长度。

![python-wzry-8](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-8.png)

这个值可以在li标签下的i标签中找到，只要取出style的值即可，这是一个百分数字符串，取这个字符串中":"和"%"之间的字符串，得到一个数字，这个数字就可以代表属性值的大小。

![python-wzry-9](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-9.png)

英雄的职业虽然没有以文本的形式显示出来，但如下`i`标签中的class值明显是和职业有关的：

![python-wzry-10](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-10.png)

只要看看其他的英雄的页面，就可以得到如下对应关系：

![python-wzry-11](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-11.png)
...
![python-wzry-12](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-12.png)
...
![python-wzry-13](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-13.png)

观察HTML代码，可以看到3大关系都放在如下3个标签中：

![python-wzry-14](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-14.png)

一个关系中有两个英雄，分别放在li标签下，而a标签下存放的路径，就指向了这个英雄：

![python-wzry-15](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-15.png)

那么依据这个链接，就可以得到这个英雄：

这里我选择在最开始获取英雄链接的函数里，加上一个建立字典的操作，这个字典的键就是路径(被分割过)，值是英雄名：

![python-wzry-16](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-16.png)
...
![python-wzry-17](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-17.png)

但到后面就报错了，显示`KeyError`：

![python-wzry-18](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-18.png)

这是因为字典中没有这个键，这说明了什么？

说明了最开始爬到的诸多英雄，是有缺失的。访问一下这个528.shtml对应的页面：

![python-wzry-19](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-19.png)

再后头检查一下爬到的英雄，发现真的没有澜...

感觉有点无语，但毕竟已经是个到这一步了，容我冷静地想一想为什么...

![while(true)反复分析(程序员)_While_true_程序员_反复_分析表情](./Python爬虫实战-获取王者荣耀英雄大全.assets/006ARE9vgy1fwemc30vd5j303y052q2r.jpg)

#### 反思问题

不难发现，这个链接返回的数据，原本就是缺的：

![python-wzry-21](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-21.png)

前面的这部分数据，明显找不到。

![python-wzry-22](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-22.png)

再看看这让人匪夷所思的URL，搞不好数据还是停留在2016年的...

再看看请求头，发现status code是304并非200，可见发生了网页重定向：

![python-wzry-23](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-23.png)
...
![python-wzry-24](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-24.png)
...
![python-wzry-25](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-25.png)
...
![python-wzry-26](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-26.png)
...
![](./Python爬虫实战-获取王者荣耀英雄大全.assets/crawler-l6-8-2019111.png)
...
![python-wzry-28](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-28.png)
...
![python-wzry-29](./Python爬虫实战-获取王者荣耀英雄大全.assets/python-wzry-29.png)
